...
def videoEnd():
	...
	if enable_debug	== "true": xbmc.log("<<<plugin.video.visszanezo (end source) "+xsource, 3)
	...
	for line in lines:
		...
		if replay == "S":
			...
			lines.insert(0, line)
			replay = "S"
			if enable_debug	== "true": xbmc.log("<<<plugin.video.visszanezo (end final replay) "+str(line), 3)
	...
	if replay=="N":
		...
		if enable_debug	== "true": xbmc.log("<<<plugin.video.visszanezo (end final play) "+str(newline), 3)

	...
	if xbmcvfs.exists(txtfile):
		...
		for line in lines:
			...
			if len(line)>3 and (group==selGroup or selGroup=='*'):
				...
				command = []
				command.append((lang(30008), "XBMC.RunPlugin(plugin://plugin.video.visszanezo?menu=remove&id="+str(idx)+")"))
...
if menu is None or menu[0]=="top":
	...
	if xbmcvfs.exists(txtfile) and lid is not None:
		...
		if len(lines)>0 or osz==1:
			...
			json.dump(lines, f)
			f.close()
		xbmc.executebuiltin("ActivateWindow(Videos,plugin://plugin.video.visszanezo?menu=top)")
